 ...
 * @file 
 *
 *   This file contains the implementation for the Extension oc9_DocRelAutoCreation
 *
 */
...
int oc9_DocRel_autocreation(METHOD_message_t *msg, va_list args) {
	try {
		...
		if(IMAN_specification_rel_type_t==NULLTAG || Oc9_KDRevision_type_t==NULLTAG || Oc9_CompanyPartRevision_type_t==NULLTAG) {
			printf("Didn't found requierd type =(");
			return ITK_ok;
		}

		int num_rels = 0;
		auto_itk_mem_free<tag_t> source_rels;
		erc = GRM_list_primary_objects_only(primary_object, IMAN_specification_rel_type_t, &num_rels, source_rels.operator tag_t **());

		if (num_rels > 0) {
			printf("found %d related primary objects", num_rels);
		...
		erc = GRM_create_relation(secondary_object, target_item, doc_rel_type,
